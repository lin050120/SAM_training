...
JA: dict[str, str] = {
    ...
    # ---- dataset grouping page ----
    "按图片手动划分 Train / Val。标准数据集目录只加载 train/val/test 下的 annotations.json，忽略旁边的备份；其他目录递归查找 COCO。也可每行指定一个 COCO 文件。"
    "现有 train/val/test 子目录会继承分组，Test锁定。导出为新目录，不更改训练路径；导出后请到「数据集登记」审核登记。":
        "画像単位で Train / Val を手動で分割します。標準データセットディレクトリでは train/val/test 配下の "
        "annotations.json のみを読み込み、隣接するバックアップは無視します。それ以外のディレクトリは COCO を"
        "再帰的に探索します。1 行に 1 つの COCO ファイルを指定することもできます。"
        "既存の train/val/test サブディレクトリはグループを引き継ぎ、Test はロックされます。"
        "新しいディレクトリへエクスポートし、学習パスは変更しません。エクスポート後は「データセット登録」で"
        "確認・登録してください。",
    "图片＋COCO来源（服务器路径，每行一个，可分批追加）":
        "画像＋COCO のソース（サーバーパス、1 行に 1 つ、分割して追加可）",
    "新批次初始分组（已知子目录优先）": "新規バッチの初期グループ（既知のサブディレクトリを優先）",
    "加载 / 追加图片": "画像を読み込み / 追加",
    "清空当前选择池（不删除文件）": "現在の選択プールをクリア（ファイルは削除しません）",
    "筛选文件名 / 路径 / 场景组（回车应用）": "ファイル名 / パス / シーングループで絞り込み（Enter で適用）",
    "显示分组": "表示するグループ",
    "页码（每页24张）": "ページ番号（1 ページ 24 枚）",
    "刷新 / 跳页": "更新 / ページ移動",
    "分组统计": "グループ集計",
    "图片与标注预览（多边形轮廓；RLE显示框）":
        "画像とアノテーションのプレビュー（ポリゴンは輪郭、RLE は矩形で表示）",
    "勾选本页图片（编号与预览一致）": "このページの画像を選択（番号はプレビューと一致）",
    "全选本页": "このページを全選択",
    "场景组名（可选，例如：书架A_上排）": "シーングループ名（任意、例：本棚A_上段）",
    "分配到": "割り当て先",
    "应用到勾选图片": "選択した画像に適用",
    "同一拍摄场景尽量放同一组。跨页操作前先应用本页选择；筛选或翻页会清空勾选。分配结果可在导出的 manual_split.json 中恢复。":
        "同じ撮影シーンはできるだけ同じグループにまとめてください。ページをまたぐ操作の前に、"
        "このページの選択を適用してください。絞り込みやページ移動で選択は解除されます。"
        "割り当て結果はエクスポートされた manual_split.json から復元できます。",
    "恢复划分名单路径（先加载相同来源）": "分割リストの復元パス（先に同じソースを読み込んでください）",
    "恢复名单": "リストを復元",
    "新数据集输出目录（必须不存在）": "新しいデータセットの出力ディレクトリ（未作成であること）",
    "导出图片及 COCO（保留原数据）": "画像と COCO をエクスポート（元データは保持）",
    "导出结果": "エクスポート結果",
